...
const StateChartData = () => {
  ...
  const stateDataURL = "http://localhost:4300/get/Count/covid/positive/statewise";
  const stateDataNegURL="http://localhost:4300/get/Count/covid/negative/statewise";
  useEffect(() => {
  ...
  }, [stateDataURL,stateDataNeg]);

  const stateData = {
    labels: stateDataNeg?.map((x) => x.state_residence),
    datasets:[
        {
        barPercentage: 0.5,
        maxBarThickness: 40,
        minBarLength: 20,
        label: `${stateDataNeg.length} Covid Negative`,
        ...
        },
        {
          ...
          maxBarThickness: 40,
          minBarLength: 20,
          label: `${stateChartData.length} Covid Positive`,
