import { Link } from "react-router-dom"
import './Breadcrumbs.css'

export default function Breadcrumbs({children}){
    return(
        <div class="breadcrumbs">
        <ul>
            <li id="home">
                <Link to="/" className="b-link" onClick={() => {window.scroll(0, 0);}}>
                    <svg xmlns="http://www.w3.org/2000/svg" class="svg-home" viewBox="0 0 20 20" fill="black">
                        <path d="M9.293 2.293a1 1 0 011.414 0l7 7A1 1 0 0117 11h-1v6a1 1 0 01-1 1h-2a1 1 0 01-1-1v-3a1 1 0 00-1-1H9a1 1 0 00-1 1v3a1 1 0 01-1 1H5a1 1 0 01-1-1v-6H3a1 1 0 01-.707-1.707l7-7z"></path>                        
                    </svg>
                    <span class="svgt">Home</span>
                </Link>
            </li>
            {children}
        </ul>
    </div>
    )
}